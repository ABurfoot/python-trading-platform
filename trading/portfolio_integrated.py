...
class IntegratedPortfolioManager:
    ...
    def withdraw(self, portfolio_name: str, amount: float, currency: str = None,
        ...
        if portfolio.cash.get(currency, 0) < amount:
            return None
    ...
    def get_summary(self, portfolio_name: str, include_tax: bool = True) -> Dict:
        ...
        # Returns
        total_return = total_portfolio_value - portfolio.initial_value
        total_return_pct = (total_return / portfolio.initial_value * 100) if portfolio.initial_value > 0 else 0
...
def main():
    ...
    if args.command == "list":
        ...
        if result:
            print(f"[OK] Sold {args.quantity} {args.symbol} @ ${args.price:.2f}")
        ...
        if result:
            print(f"[OK] Recorded dividend: ${args.amount:.2f} from {args.symbol}")
        ...
        if prices:
            print(f"[OK] Updated {len(prices)} prices")
